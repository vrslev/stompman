...
def dump_frame(frame: ClientFrame | AnyServerFrame) -> bytes:
    lines = (
...
def parse_lines_into_frame(lines: deque[list[bytes]]) -> ClientFrame | AnyServerFrame | None:
...
class Parser:
    ...
    def load_frames(self, raw_frames: bytes) -> Iterator[ClientFrame | AnyServerFrame | HeartbeatFrame]:
